...
public class Admin extends User {
    ...
    public Admin(String id, String name, String email, String password, AccountStatus status, int roleLevel) {
        setId(id); setName(name); setEmail(email); setPassword(password); setStatus(status); this.roleLevel = roleLevel;
    }
    ...
    @Override public String toCsvLine() {
        return String.join(",", getId(), CsvUtil.escapeCsv(getName()), getEmail(), getPassword(), getStatus().name(), String.valueOf(roleLevel));
    }
    @Override public void fromCsvLine(String line) {
        ...
        setStatus(AccountStatus.valueOf(parts[4].trim())); this.roleLevel = Integer.parseInt(parts[5].trim());
    }
}
